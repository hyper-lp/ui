...
async function testTokenPrice(tokenName: string, address: string, chainId: number, expectedBehavior: string) {
    ...
    try {
        ...
        if (price > 0) {
            ...
            if (tokenName.includes('HYPE') && chainId === 998) {
                console.log(`  ${colors.green}ℹ${colors.reset} Source: Oracle/Perpetuals API`)
            }
        } else if (price === 0 && (chainId !== 998 || tokenName === 'UNKNOWN')) {
...
async function testCaching() {
    ...
    const price1 = await getTokenPrice(TEST_TOKENS.HYPE as `0x${string}`, 998)
    ...
    const price2 = await getTokenPrice(TEST_TOKENS.HYPE as `0x${string}`, 998)
    ...
    const price3 = await getTokenPrice(TEST_TOKENS.HYPE as `0x${string}`, 998)
...
async function testBatchRequests() {
    ...
    const requests = [
        getTokenPrice(TEST_TOKENS.HYPE as `0x${string}`, 998),
        getTokenPrice(TEST_TOKENS.WHYPE as `0x${string}`, 998),
        getTokenPrice(TEST_TOKENS.USDT0 as `0x${string}`, 998),
        getTokenPrice(TEST_TOKENS.feUSD as `0x${string}`, 998),
    ]
...
async function main() {
    ...
    await testTokenPrice('HYPE', TEST_TOKENS.HYPE, 998, 'Live price from Hyperliquid')
    await testTokenPrice('WHYPE', TEST_TOKENS.WHYPE, 998, 'Live price from Hyperliquid')
    await testTokenPrice('USDT0', TEST_TOKENS.USDT0, 998, 'Fixed at $1.00')
    await testTokenPrice('feUSD', TEST_TOKENS.feUSD, 998, 'Fixed at $1.00')
    await testTokenPrice('Unknown Token', TEST_TOKENS.UNKNOWN, 998, 'Should return 0')
