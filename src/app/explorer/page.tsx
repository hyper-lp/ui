...
export default function ExplorerPage() {
    ...
    const deltaHistory = address ? getHistory(address) : null
    ...
    useEffect(() => {
        if (data?.summary && address) {
            ...
            addDataPoint(address, point)
        }
    }, [data?.summary, address, addDataPoint])
    ...
    return (
        <PageWrapper>
            ...
            {/* Show content only if we have a valid address */}
            {!isValidAddress ? (
                <div className="py-8 text-center text-gray-500">Enter a valid EVM address to explore account positions and balances</div>
            ) : isLoading && !data ? (
                // Loading screen
                <div className="py-8">
                    <div className="mx-auto max-w-2xl space-y-2 font-mono text-sm">
                        {loadingSteps.map((step, index) => {
                            const currentTime = Date.now() + timerTick * 0
                            const getStatus = () => {
                                if (step.status === 'done') {
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-blue-500">
                                        ...
                                        {(() => {
                                            ...
                                            const finalizingTime = lastStep?.endTime || Date.now()
                                            return ((Date.now() - finalizingTime) / 1000).toFixed(1)
                                        })()}
                                        s)
                                    </span>
                                </div>
                    ...
                    {(() => {
                        ...
                        return (
                            <>
                                ...
                                >
                                    {data.summary && deltaHistory && <StrategyMonitoring summary={data.summary} deltaHistory={deltaHistory} />}
                                </CollapsibleSection>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {data.positions?.lp && data.positions.lp.length > 0 && (
                                            <div>
                                                ...
                                                <pre className="overflow-x-auto rounded bg-gray-100 p-2 text-xs">
                                                    {JSON.stringify(data.positions.lp, null, 2)}
                                                </pre>
                                            </div>
                                        )}
                                        ...
                                        {data.positions?.perp && data.positions.perp.length > 0 && (
                                            <div>
                                                ...
                                                <pre className="overflow-x-auto rounded bg-gray-100 p-2 text-xs">
                                                    {JSON.stringify(data.positions.perp, null, 2)}
                                                </pre>
                                            </div>
                                        )}
                                        ...
                                        {data.positions?.spot && data.positions.spot.length > 0 && (
                                            <div>
                                                ...
                                                <pre className="overflow-x-auto rounded bg-gray-100 p-2 text-xs">
                                                    {JSON.stringify(data.positions.spot, null, 2)}
                                                </pre>
                                            </div>
                                        )}
