 ...
 * @param marketAddress The market address (e.g., "0x6fCD93da1E2A288AE68bb1D4f856d8D598E8B861")
 * @returns Current APR/APY data including 7d and 28d averages
 */
export async function fetchHyperDriveAPR(marketAddress: string): Promise<HyperDriveCurrentAPR | null> {
    try {
 ...
 */
export async function getFormattedHyperDriveAPR(marketAddress: string): Promise<string | null> {
    const aprData = await fetchHyperDriveAPR(marketAddress)
...
export async function getAllHyperDriveAPRs(): Promise<Map<string, HyperDriveCurrentAPR>> {
    const { HYPEREVM_PROTOCOLS, ProtocolType } = await import('@/config/hyperevm-protocols.config')
    const hyperDriveConfig = HYPEREVM_PROTOCOLS[ProtocolType.HYPERDRIVE]
    
    const aprMap = new Map<string, HyperDriveCurrentAPR>()
    
    if (!hyperDriveConfig?.lendingConfig?.markets) {
        return aprMap
    }

    // Fetch APR for each market in parallel
    const promises = hyperDriveConfig.lendingConfig.markets.map(async (market) => {
        const apr = await fetchHyperDriveAPR(market.address)
        if (apr) {
            aprMap.set(market.address.toLowerCase(), apr)
        }
    })
